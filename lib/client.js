      ...
      return function WorkbenchPanel() {
        ...
        React.useEffect(function () {
          ...
          return function () {
            ...
            var popouts = popoutsRef.current;
            popoutsRef.current = [];
            for (var index = 0; index < popouts.length; index += 1) {
              try {
                if (popouts[index] && !popouts[index].closed) popouts[index].close();
              } catch (error) { /* popup may already be unavailable */ }
            }
          };
        }, []);
          ...
          React.createElement('div', { key: 'bar', style: styles.bar }, [
            ...
            React.createElement('span', { key: 'actions', style: styles.actions }, [
              React.createElement('button', {
                ...
                onClick: function () {
                  try {
                    // Keep the WindowProxy so Panel unmount can close the same-origin popout.
                    // Chromium returns null for window.open(..., 'noopener'), which would
                    // leave the popout alive after the Harness Panel is closed.
                    var popup = window.open(workbenchUrl, '_blank');
                    if (popup) popoutsRef.current.push(popup);
                  } catch (error) { /* ignore */ }
                },
