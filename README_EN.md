...
The published version is `v0.5.3`; it polishes the workbench as the Quiet Studio visual system, organizes the project and test entry points, and fixes page selection being overwritten during application initialization while retaining Harness's official Panel/Slot API and `dsh-v0.1.7-alpha.1` compatibility. Runtime verification of the independent `quality-control` bundle remains unassessed.
...
lib/client.js     Browser half (0.5.3): official Panel/Slot sidebar + main keyed slot + Workbench iframe
